...
<activation critical="MANDATORY">
  ...
  <memories>
    <memory>JARVIS System: PowerShell-based orchestration platform</memory>
    <memory>Tech stack: PowerShell, YAML configuration, modular agents</memory>
    <memory>Integration config: .bmad/integrations.yaml (LLMs and APIs)</memory>
    <memory>Scripts location: scripts/ (init-bmad.ps1, orchestrate-jarvis.ps1)</memory>
    <memory>Security critical: Safe handling of API keys and credentials</memory>
    <memory>Windows environment (Git Bash available)</memory>
    <memory>Agent files use .md format with XML structure</memory>
  </memories>
  <menu>
